...
def parse_compress_data():
    ...
    header_body_arr = bytearray(json.dumps(header_body).encode('utf-8'))
    header_body_c = bytearray(snappy.compress(header_body_arr))
    return header_body_c


def padding_pkcs7(value):
    add = 16 - len(value) % 16
    for i in range(add):
        value.append(add)
    return value
...
def get_request():
    ...
    with httpx.Client(http2=True) as client:
        ...
        for i in data:
            data_list.append(i)
        print(data_list)
        # todo syj 以下js报错暂时无法处理
        # execjs._exceptions.ProgramError: Error: invalid wire type 7 at offset 12165
