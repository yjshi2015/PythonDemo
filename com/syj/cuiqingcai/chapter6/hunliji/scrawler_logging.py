...
async def get_data():
    # begin_id = 68809123
    tasks = []
    for card_id in range(60400001, 60400100):
    # for card_id in range(60400014, 60400016):
        ...
        logging.info('card_id: %s, card_id_base64: %s', card_id_str, card_id_base64)
    ...
    for response_content in results:
        try:
            ...
            if status_info['retCode'] != 0:
                logging.error('response status error: (%s)', status_info)
                continue
            ...
            logging.error('parse result error, card_id:' + response_content.get('data').get('cardId'))
            print(e)
